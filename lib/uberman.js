...
function uberman (configs) {
  ...
  app.configs.port = configs.port || process.env.PORT || 443;
  app.configs.env = configs.env || process.env.ENV || 'dev';
  app.configs.version = configs.version || process.env.VERSION || 0;
  ...
  var addEndpoint = function addEndpoint (name, schema, options) {
