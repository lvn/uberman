...
function uberman (configs) {
  ...
  // connect to db (mongoDb by default)
  app.db = app.configs ? 
    require(app.configs.db.adapter).connect(app.configs.db) ||
    require('./adapters/mongodb').connect({
      host: 'localhost',
      port: 27017,
      database: app.configs.name
    });
  ...
  // attach db adapter to every request.
  var attachDbAdapter = function attach (req, res, next) {
    req.db = app.db;
    return next();
  };
